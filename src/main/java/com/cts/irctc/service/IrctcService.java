...
public class IrctcService {
	...
	public TrainInfo getTicketBookingResult(TicketBooking ticketBooking) throws ApplicationException {
		...
		if(ticketBooking.getFromCity().equalsIgnoreCase(ticketBooking.getToCity())) {
			throw new ApplicationException("From and to city can't be same");
		}
		else {
			...
			for(TrainInfo trainInfo:list1) {
				if(trainInfo.getTavelClass().equalsIgnoreCase(trv)&& trainInfo.getFrom().equalsIgnoreCase(ticketBooking.getFromCity())&& trainInfo.getTo().equalsIgnoreCase(ticketBooking.getToCity()));
				{
					return trainInfo;
				}
			}
